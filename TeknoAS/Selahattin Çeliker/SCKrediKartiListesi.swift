...
class SCKrediKartiViewModel: ObservableObject {
    @Published var showAddSheet = false
    @Published var krediKartiList: [SCKrediKarti]
    ...
    func fetchKrediKartlari() {
        do {
            let descriptor = FetchDescriptor<SCKrediKarti>(
                sortBy: [SortDescriptor(\.eklemeTarihi, order: .reverse)]
            )
            krediKartiList = try modelContext.fetch(descriptor)
        } catch {
            print("❌ SC kredi kartları çekilemedi: \(error)")
        }
    }
    
    var filteredKartlar: [SCKrediKarti] {
    ...
    func getKartShareText(kart: SCKrediKarti) -> String {
        """
    ...
    private func saveContext() {
        do {
            ...
            print("❌ SC kayıt başarısız: \(error)")
        }
    }
}

struct SCKrediKartiListesi: View {
    @Environment(\.modelContext) var modelContext
    @StateObject private var viewModel: SCKrediKartiViewModel
    @State private var editMode: EditMode = .inactive
    @State private var selectedKart: SCKrediKarti?
    @State private var showDetail = false

    init(modelContext: ModelContext) {
        _viewModel = StateObject(wrappedValue: SCKrediKartiViewModel(modelContext: modelContext))
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $viewModel.showAddSheet) {
                SCKrediKartiEkle(viewModel: viewModel)
            }
            .sheet(item: $selectedKart) { kart in
                SCKrediKartiDetayView(kart: kart, viewModel: viewModel)
            }
        }
    }
            ...
            ForEach(viewModel.filteredKartlar) { kart in
                if editMode == .inactive {
                    Button(action: {
                        ...
                        SCKrediKartiCardView(kart: kart, masked: viewModel.maskedCards)
                    }
                    .listRowBackground(
                    ...
                    SCKrediKartiEditView(kart: kart)
                }
            }
...
// MARK: - Kart Görünümü
struct SCKrediKartiCardView: View {
    let kart: SCKrediKarti
    let masked: Bool
...
struct SCKrediKartiEditView: View {
    let kart: SCKrediKarti
...
// MARK: - Detay Görünümü
struct SCKrediKartiDetayView: View {
    let kart: SCKrediKarti
    let viewModel: SCKrediKartiViewModel
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    VStack(spacing: 16) {
                        scDetayBilgi(baslik: "Şirket", icerik: kart.sirketAdi, ikon: "building.2")
                        scDetayBilgi(baslik: "Kart Sahibi", icerik: kart.kartSahibi, ikon: "person")
                        scDetayBilgi(baslik: "Banka", icerik: kart.bankaAdi, ikon: "building.columns")
                        scDetayBilgi(baslik: "Kart Tipi", icerik: kart.kartTipi, ikon: "creditcard")
                        
                        Divider()
                        
                        scGizlenebilirBilgi(baslik: "Kart Numarası", icerik: kart.kartNumarasi, ikon: "number")
                        scGizlenebilirBilgi(baslik: "Son Kullanma", icerik: kart.sonKullanmaTarihi, ikon: "calendar")
                        scGizlenebilirBilgi(baslik: "CVV", icerik: kart.cvv, ikon: "lock")
                        
                        if !kart.notlar.isEmpty {
                            Divider()
                            scDetayBilgi(baslik: "Notlar", icerik: kart.notlar, ikon: "note.text")
                        }
                    }
                    .padding()
    ...
    private func scDetayBilgi(baslik: String, icerik: String, ikon: String) -> some View {
        HStack(spacing: 16) {
    ...
    private func scGizlenebilirBilgi(baslik: String, icerik: String, ikon: String) -> some View {
        HStack(spacing: 16) {
            Image(systemName: ikon)
                .font(.title3)
                .foregroundStyle(.red)
...
#Preview {
    SCKrediKartiListesi(modelContext: ModelContext(try! ModelContainer(for: SCKrediKarti.self)))
}
