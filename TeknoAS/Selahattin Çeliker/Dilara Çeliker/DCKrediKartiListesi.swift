...
class DCKrediKartiViewModel: ObservableObject {
    @Published var showAddSheet = false
    @Published var krediKartiList: [DCKrediKarti]
    ...
    func fetchKrediKartlari() {
        do {
            let descriptor = FetchDescriptor<DCKrediKarti>(
                sortBy: [SortDescriptor(\.eklemeTarihi, order: .reverse)]
            )
            krediKartiList = try modelContext.fetch(descriptor)
        } catch {
            print("❌ DC kredi kartları çekilemedi: \(error)")
        }
    }
    
    var filteredKartlar: [DCKrediKarti] {
    ...
    func getKartShareText(kart: DCKrediKarti) -> String {
        """
    ...
    private func saveContext() {
        do {
            ...
            print("❌ DC kayıt başarısız: \(error)")
        }
    }
}

struct DCKrediKartiListesi: View {
    @Environment(\.modelContext) var modelContext
    @StateObject private var viewModel: DCKrediKartiViewModel
    @State private var editMode: EditMode = .inactive
    @State private var selectedKart: DCKrediKarti?
    @State private var showDetail = false

    init(modelContext: ModelContext) {
        _viewModel = StateObject(wrappedValue: DCKrediKartiViewModel(modelContext: modelContext))
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $viewModel.showAddSheet) {
                DCKrediKartiEkle(viewModel: viewModel)
            }
            .sheet(item: $selectedKart) { kart in
                DCKrediKartiDetayView(kart: kart, viewModel: viewModel)
            }
        }
    }
            ...
            ForEach(viewModel.filteredKartlar) { kart in
                if editMode == .inactive {
                    Button(action: {
                        ...
                        DCKrediKartiCardView(kart: kart, masked: viewModel.maskedCards)
                    }
                    .listRowBackground(
                    ...
                    DCKrediKartiEditView(kart: kart)
                }
            }
...
// MARK: - Kart Görünümü
struct DCKrediKartiCardView: View {
    let kart: DCKrediKarti
    let masked: Bool
...
struct DCKrediKartiEditView: View {
    let kart: DCKrediKarti
...
// MARK: - Detay Görünümü
struct DCKrediKartiDetayView: View {
    let kart: DCKrediKarti
    let viewModel: DCKrediKartiViewModel
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    VStack(spacing: 16) {
                        dcDetayBilgi(baslik: "Şirket", icerik: kart.sirketAdi, ikon: "building.2")
                        dcDetayBilgi(baslik: "Kart Sahibi", icerik: kart.kartSahibi, ikon: "person")
                        dcDetayBilgi(baslik: "Banka", icerik: kart.bankaAdi, ikon: "building.columns")
                        dcDetayBilgi(baslik: "Kart Tipi", icerik: kart.kartTipi, ikon: "creditcard")
                        
                        Divider()
                        
                        dcGizlenebilirBilgi(baslik: "Kart Numarası", icerik: kart.kartNumarasi, ikon: "number")
                        dcGizlenebilirBilgi(baslik: "Son Kullanma", icerik: kart.sonKullanmaTarihi, ikon: "calendar")
                        dcGizlenebilirBilgi(baslik: "CVV", icerik: kart.cvv, ikon: "lock")
                        
                        if !kart.notlar.isEmpty {
                            Divider()
                            dcDetayBilgi(baslik: "Notlar", icerik: kart.notlar, ikon: "note.text")
                        }
                    }
                    .padding()
    ...
    private func dcDetayBilgi(baslik: String, icerik: String, ikon: String) -> some View {
        HStack(spacing: 16) {
    ...
    private func dcGizlenebilirBilgi(baslik: String, icerik: String, ikon: String) -> some View {
        HStack(spacing: 16) {
            Image(systemName: ikon)
                .font(.title3)
                .foregroundStyle(.red)
...
#Preview {
    DCKrediKartiListesi(modelContext: ModelContext(try! ModelContainer(for: DCKrediKarti.self)))
}
