...
class TeknoKrediKartiViewModel: ObservableObject {
    @Published var showAddSheet = false
    @Published var krediKartiList: [TeknoKrediKarti]
    @Published var searchText = ""
    @Published var maskedCards = true // Kart numaralarını gizle/göster
    ...
    func fetchKrediKartlari() {
        do {
            let descriptor = FetchDescriptor<TeknoKrediKarti>(
                sortBy: [SortDescriptor(\.eklemeTarihi, order: .reverse)]
            )
            krediKartiList = try modelContext.fetch(descriptor)
        } catch {
            print("Kredi kartları çekilemedi: \(error)")
        }
    }
    
    var filteredKartlar: [TeknoKrediKarti] {
    ...
    func getKartShareText(kart: TeknoKrediKarti) -> String {
        """
...
struct TeknoKrediKartiListesi: View {
    @Environment(\.modelContext) var modelContext
    @StateObject private var viewModel: TeknoKrediKartiViewModel
    @State private var editMode: EditMode = .inactive
    @State private var selectedKart: TeknoKrediKarti?
    @State private var showDetail = false

    init(modelContext: ModelContext) {
        _viewModel = StateObject(wrappedValue: TeknoKrediKartiViewModel(modelContext: modelContext))
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $viewModel.showAddSheet) {
                TeknoKrediKartiEkle(viewModel: viewModel)
            }
            .sheet(item: $selectedKart) { kart in
                KrediKartiDetayView(kart: kart, viewModel: viewModel)
            }
        }
    }
            ...
            ForEach(viewModel.filteredKartlar) { kart in
                if editMode == .inactive {
                    Button(action: {
                        ...
                        KrediKartiCardView(kart: kart, masked: viewModel.maskedCards)
                    }
                    .listRowBackground(
...
// MARK: - Kart Görünümü
struct KrediKartiCardView: View {
    let kart: TeknoKrediKarti
    let masked: Bool
...
struct KrediKartiEditView: View {
    let kart: TeknoKrediKarti
...
struct KrediKartiDetayView: View {
    let kart: TeknoKrediKarti
    let viewModel: TeknoKrediKartiViewModel
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    // Detay Bilgileri
                    VStack(spacing: 16) {
                        detayBilgi(baslik: "Şirket", icerik: kart.sirketAdi, ikon: "building.2")
                        detayBilgi(baslik: "Kart Sahibi", icerik: kart.kartSahibi, ikon: "person")
                        detayBilgi(baslik: "Banka", icerik: kart.bankaAdi, ikon: "building.columns")
                        detayBilgi(baslik: "Kart Tipi", icerik: kart.kartTipi, ikon: "creditcard")
                        
                        Divider()
                        
                        gizlenebilirBilgi(baslik: "Kart Numarası", icerik: kart.kartNumarasi, ikon: "number")
                        gizlenebilirBilgi(baslik: "Son Kullanma", icerik: kart.sonKullanmaTarihi, ikon: "calendar")
                        gizlenebilirBilgi(baslik: "CVV", icerik: kart.cvv, ikon: "lock")
                        
                        if !kart.notlar.isEmpty {
                            Divider()
                            detayBilgi(baslik: "Notlar", icerik: kart.notlar, ikon: "note.text")
                        }
                    }
                    .padding()
    ...
    private func detayBilgi(baslik: String, icerik: String, ikon: String) -> some View {
        HStack(spacing: 16) {
    ...
    private func gizlenebilirBilgi(baslik: String, icerik: String, ikon: String) -> some View {
        HStack(spacing: 16) {
            Image(systemName: ikon)
                .font(.title3)
                .foregroundStyle(.red)
...
#Preview {
    TeknoKrediKartiListesi(modelContext: ModelContext(try! ModelContainer(for: TeknoKrediKarti.self)))
}
